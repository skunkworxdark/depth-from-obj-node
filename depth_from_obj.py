...
class DepthMapFromWavefrontObjInvocation(BaseInvocation, WithMetadata, WithBoard):
    ...
    rotate_x: int = InputField(
        default=20, description="Degrees by which to tip the model about the x-axis after y-rotation"
    )
    rotate_y: int = InputField(
        default=45, description="Degrees by which to rotate the model's bearing about the y-axis"
    )
    ...
    def translate(self, x, y, z):
        return numpy.array([[1, 0, 0, x], [0, 1, 0, y], [0, 0, 1, z], [0, 0, 0, 1]], dtype=float)

    def rotation_x(self, theta):
        theta_radians = numpy.pi * theta / 180
        cosine, sine = numpy.cos(theta_radians), numpy.sin(theta_radians)
        return numpy.array([[1, 0, 0, 0], [0, cosine, -1 * sine, 0], [0, sine, cosine, 0], [0, 0, 0, 1]], dtype=float)

    def rotation_y(self, theta):
        theta_radians = numpy.pi * theta / 180
        cosine, sine = numpy.cos(theta_radians), numpy.sin(theta_radians)
        return numpy.array([[cosine, 0, sine, 0], [0, 1, 0, 0], [-1 * sine, 0, cosine, 0], [0, 0, 0, 1]], dtype=float)
    ...
    def invoke(self, context: InvocationContext) -> ImageOutput:
        ...
        matplotlib.pyplot.savefig(image_buffer, facecolor=(0, 0, 0, 1), format="png", dpi=max(self.width, self.height))
